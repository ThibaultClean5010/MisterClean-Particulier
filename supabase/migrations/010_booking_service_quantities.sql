-- Adds quantities for repeated items of the same service while preserving one
-- booking_services row per distinct service.
alter table public.booking_services
  add column quantity smallint not null default 1 check (quantity between 1 and 10);

create or replace function public.get_booking_availability_quantities(
  p_services jsonb,
  p_local_date date
)
...
begin
  ...
  v_requested_count := jsonb_array_length(p_services);
  ...
  if exists (
    select 1
    from jsonb_to_recordset(p_services) as requested(service_id uuid, quantity integer)
    where service_id is null or quantity is null or quantity not between 1 and 10
  ) or (
...
begin
  ...
  if found then
    select coalesce(
      array_agg(case when bs.quantity > 1 then bs.quantity::text || ' × ' || bs.service_name else bs.service_name end order by bs.position),
      array[v_existing.service_name]
  ...
  select
    count(*),
    sum(s.duration_minutes * requested.quantity)::integer,
    max(s.buffer_after_minutes),
    case when count(s.price_cents) = count(*) then sum(s.price_cents * requested.quantity)::integer end,
    count(s.price_cents)::integer,
    string_agg(
      case when requested.quantity > 1 then requested.quantity::text || ' × ' || s.name else s.name end,
  ...
  if not exists (
    select 1 from public.get_booking_availability_quantities(p_services, v_local_date) a
  ...
  insert into public.booking_services (
    booking_id, service_id, position, service_name, quantity, duration_minutes, price_cents, price_label
  )
  select
    v_booking_id, s.id,
    (row_number() over (order by s.sort_order, s.name))::smallint,
    s.name, requested.quantity, s.duration_minutes, s.price_cents, s.price_label
  from jsonb_to_recordset(p_services) as requested(service_id uuid, quantity integer)
  join public.services s on s.id = requested.service_id
  order by s.sort_order, s.name;
